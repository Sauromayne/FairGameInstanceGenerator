...
def create_master_instances(json_dict, directory, num_asins):
    ...
    while current_instance <= num_instances:
        ...
        return_list = create_master_json(
            current_fg_dir,
            asin_list,
            reserves_list,
            json_dict["domain"],
        ...
        if not os.path.exists(
            os.path.join(directory, f"start_all_{instance_name}.bat")
        ):
            start_bat = open(
                os.path.join(directory, f"start_all_{instance_name}.bat"), "a"
            )
...
def create_dev_instances(json_dict, directory, num_asins):
    ...
    while current_instance <= num_instances:
        ...
        # folder copied, insert new json into the correct Fairgame directory
        current_fg_dir = os.path.join(directory, f"{current_instance}", "config")
        return_list = create_dev_json(
            current_fg_dir, asin_list, reserves_list, json_dict, int(num_asins)
        )
...
if __name__ == "__main__":
    ...
    print(
        f'!*!*YOUR REFERENCE FOLDER SHOULD BE NAMED "fairgame-dev" FOR DEV BRANCH!*!*'
    )
    print(
        f'!*!*YOUR REFERENCE FOLDER SHOULD BE NAMED "fairgame-alpha" FOR ALPHA BRANCH!*!*'
    )
    print("-" * 80)
    ...
    if alpha_instance and os.path.exists(
        os.path.join(CONFIG_DIR, "alpha", "alpha.json")
    ):
        ...
        print(
            "No valid alpha JSON file is present. Please close the program and look in your config/alpha folder. "
    ...
    if os.path.exists(os.path.join(CONFIG_DIR, "master.json")):
        ...
        print(
            "Creating instances can take time depending on your PC's specs, so have patience..."
        )
        ...
        print(
            "No valid JSON file is present. Please close the program and look in your config folder. "
    ...
    if not invalid_json and not alpha_instance:
        print("-" * 80)
        print(
            "DONE! Run the mass_install.bat file in your instance directory to install all instances."
        )
        print(
            "Run the start_all.bat in your instance directory to start all instances."
        )
        print("Starting many instances at once can take some time, have patience!")
        print(
            "When closing your instances, use Ctrl + C to kill the instances before closing them."
        )

    if alpha_instance:
        print(
            "Make sure to run the INSTALL.bat file in your new instance directory in order to install it."
        )
        print(
            "Once you have completed installation your instance is ready to run with _Amazon_aio.bat"
        )
